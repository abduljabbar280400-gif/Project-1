...
export default function DriverPayouts() {
  ...
  const fetchPayoutData = async () => {
    ...
    try {
      const data = await api.delivery.getPayouts();
      setEarningsBalance(data.earnings_balance || 0);
      setPayouts(data.payouts?.data || data.payouts || []);
      setCompletedDeliveries(data.completed_deliveries?.data || data.completed_deliveries || []);
    } catch (err) {
  ...
  const handlePayoutSubmit = async (e) => {
    ...
    setFormLoading(true);
    try {
      const res = await api.delivery.requestPayout({ amount: amt, bank_name: bankName, account_number: accountNumber, account_name: accountName });
  ...
  if (loading) return <LoadingSpinner message="Retrieving driver earnings..." />;
  ...
  return (
    <div className="space-y-6 pb-8 select-none">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="card-solid p-4 space-y-2 border-t-4 border-t-emerald-500">
          ...
          <button
            onClick={() => setIsModalOpen(true)} disabled={earningsBalance <= 0}
      ...
      <div className="space-y-3">
        <span className="text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Completed Jobs Settlement</span>
        {completedDeliveries.length === 0 ? (
          <p className="text-xs italic" style={{ color: 'var(--text-muted)' }}>No settled deliveries yet.</p>
        ) : (
          <div className="space-y-2 max-h-48 overflow-y-auto rounded-2xl p-2" style={{ backgroundColor: 'var(--bg-panel)', border: '1px solid var(--border)' }}>
            {completedDeliveries.map(order => (
              <div key={order.id} className="flex justify-between items-center text-xs p-2 rounded-lg">
                <div>
                  <span className="font-extrabold block" style={{ color: 'var(--text-head)' }}>Job #{order.id}</span>
                  <span className="text-[10px] font-semibold" style={{ color: 'var(--text-muted)' }}>{order.restaurant_name} &rarr; {order.delivery_address}</span>
                </div>
                <span className="font-bold text-emerald-600">+₹{Number(order.delivery_fee).toFixed(2)}</span>
              </div>
            ))}
          </div>
        )}
